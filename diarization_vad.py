...
class baseline_ASD_VAD(object):
    def __init__(self, GPU='3', code_path='/workspace/great_src21_converse/train_dist', \
    model_path='/nvme1/zhiyong/ASV_LOGS_202102/train_logs_201120/sdsv21_full_pretrain/model/model000000024.model'):
        # extract embs
        # print('extract embs')

        # segment to 1.5s
        # print('segment to 1.5s')
        self.seg_len = int(1.5 * 16000)
        # self.seg_hop = int(0.75 * 16000)
        self.seg_hop = int(0.25 * 16000)
        self.TH = 0.98
        ...
        self.S = SpeakerNetModel(n_mels=40, nOut=192, spec_aug=False)

        self.S.cuda()
        loaded_state = torch.load(model_path, map_location="cuda:0")

        self_state = self.S.state_dict()
    ...
    def audio2emb(self, audio):
        audiosize = audio.shape[0]
        max_audio = self.seg_len
        ...
        ref_feat = self.S.forward(inp1).detach().cpu()
        return ref_feat

    def SC(self, inp_matrix):
        ...
        return sp_kmeans.labels_, lam

    def extract(self, inputfile):
        # cal vad
        # print('calculate vad')
        res = m_mfcc_vad.m_mfcc(inputfile, alpha=0.02, k=0.5, ou=50)
        sad = []
        for i in res:
        ...
        for j in perspk_dict_tmpinsample:
            if perspk_dict_tmpinsample[j] == 'unknown':
                ...
                if (end - start) <= self.seg_len:
                    perspk_dict_tmpinsample_seg[str(start)+'.'+str(end)] = 'unknown'
                else:
                    start_tmp = start
                    end_tmp = start + self.seg_len
                    assert end_tmp < end
                    while(True):
                        if (end_tmp+self.seg_hop) > end:
                            end_tmp = end                    
                            perspk_dict_tmpinsample_seg[str(start_tmp)+'.'+str(end_tmp)] = 'unknown'
                            break
                        else:
                            perspk_dict_tmpinsample_seg[str(start_tmp)+'.'+str(end_tmp)] = 'unknown'
                            start_tmp = start_tmp + self.seg_hop
                            end_tmp = end_tmp + self.seg_hop
        ...
        for j in range(len(matind2time)):
            ...
            start, end = int(start), int(end)
            if embs is not None:
                embs = torch.cat([embs, self.audio2emb(whole_wav[start:end])], dim=0)
            else:
                embs = self.audio2emb(whole_wav[start:end])
        ...
        spk_clustering_result, lam = self.SC(inp_matrix)
        ...
        for i in tmpinsample2:
            line = str(float(i.split('.')[0])/16000)+' '+str(float(i.split('.')[1])/16000)+' '+tmpinsample2[i]
            output.append(line)
        
        return output, lam


# def baseline_ASD_VAD(inputfile, GPU='3', code_path='/workspace/GREAT_ASV_system/train_dist', \
...
#     #print('read wav and segment with vad')
...
#     inp_matrix = (similarity + 1.0) / 2.0
#     spk_clustering_result, lam = SC(inp_matrix)
...
if __name__ == '__main__':
    print(baseline_ASD_VAD('test.wav'))
